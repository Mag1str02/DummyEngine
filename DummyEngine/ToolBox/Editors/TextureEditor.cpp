...
namespace DummyEngine {
    ...
    Ref<TextureData> TextureEditor::GetTexturePart(const Ref<TextureData> data, U32 width, U32 height, U32 x_pos, U32 y_pos) {
        ...
        U32         pixel_size = data->PixelSize();
        void*       new_image  = res->Data();
        const void* old_image  = data->Data();
        for (size_t i = 0; i < height; ++i) {
            std::memcpy(
                (U8*)new_image + (i * width * pixel_size), (U8*)old_image + (data->Width() * (i + y_pos) + x_pos) * pixel_size, width * pixel_size);
        }
        return res;
    }
