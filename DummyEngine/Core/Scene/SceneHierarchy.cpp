...
namespace DE {
    ...
    Ref<SceneHierarchyNode> SceneHierarchyNode::Detach() {
        return m_Parent->DetachChild(this);
    }

    SceneHierarchyNode* SceneHierarchyNode::GetParent() const {
        return m_Parent;
    }
    Entity SceneHierarchyNode::GetEntity() const {
        return m_Entity;
    }
    const std::string& SceneHierarchyNode::GetName() {
        if (m_Entity.Valid()) {
            return m_Entity.Get<TagComponent>().Get();
        }
        return m_Name;
    }
    void SceneHierarchyNode::SetName(const std::string& name) {
        m_Name = name;
    }
    void SceneHierarchyNode::SetEntity(const Entity& entity) {
        m_Entity = entity;
    }
    bool SceneHierarchyNode::IsEntity() const {
        return m_Entity.Valid();
    }
    bool& SceneHierarchyNode::Selected() {
        return m_Selected;
    }
    ...
    std::vector<Ref<SceneHierarchyNode>>::iterator SceneHierarchyNode::begin() {
        return m_Childs.begin();
    }
    std::vector<Ref<SceneHierarchyNode>>::iterator SceneHierarchyNode::end() {
        return m_Childs.end();
    }
