...
namespace DE {
    ...
    bool EntityManager::Valid(uint32_t id, uint32_t gen) const {
        return m_States[id] && m_Generations[id] == gen;
    }
    ...
    uint32_t EntityManager::Generation(uint32_t id) const {
    ...
    uint32_t EntityManager::EndEntity() const {
        return m_States.size();
    }
#endif
}  // namespace DE