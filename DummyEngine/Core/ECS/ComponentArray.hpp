...
namespace DE
{
    ...
    {
        DE_ASSERT(!m_EntityToIndex.contains(id), "Adding component which already exists");
    ...
    {
        DE_ASSERT(m_EntityToIndex.contains(id), "Accessing non-existing component");
        return &m_ComponentArray[m_EntityToIndex[id]];
    }
    template <typename ComponentType> bool ComponentArray<ComponentType>::HasComponent(uint32_t id) { return m_EntityToIndex.contains(id); }
    template <typename ComponentType> void ComponentArray<ComponentType>::RemoveComponent(uint32_t id)
    {
        DE_ASSERT(m_EntityToIndex.contains(id), "Removing non-existing component");
