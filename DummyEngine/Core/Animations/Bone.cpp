...
namespace DE {
    Bone::Bone(const std::string& name, U32 ID) : m_Name(name), m_ID(ID), m_LocalTransform(1.0f) {}
    ...
    U32 Bone::GetPositionIndex(float time) {
        for (U32 i = 0; i < m_Positions.size() - 1; ++i) {
    ...
    U32 Bone::GetRotationIndex(float time) {
        for (U32 i = 0; i < m_Rotations.size() - 1; ++i) {
    ...
    U32 Bone::GetScaleIndex(float time) {
        for (U32 i = 0; i < m_Scales.size() - 1; ++i) {
