...
namespace DE
{
    ...
    public:
        ...
        void DestroyEntity(EntityId id) { m_EntityManager.DestroyEntity(id); }

        template <typename ComponentType> ComponentType& AddComponent(EntityId id, ComponentType component = ComponentType())
        {
        ...
        {
            DE_ASSERT(m_ComponentManager.GetComponentId<ComponentType>() != -1, "Attempt to access non existing component arry.");
            DE_ASSERT(m_ComponentManager.GetComponentArray<ComponentType>()->HasComponent(id), "Attempt to delete non existing component.");
        ...
        {
            DE_ASSERT(m_ComponentManager.GetComponentId<ComponentType>() != -1, "Attempt to access non existing component array.");
            return m_ComponentManager.GetComponent<ComponentType>(id);
        }
        template <typename ComponentType> bool HasComponent(EntityId id) { return m_ComponentManager.GetComponentArray<ComponentType>()->HasComponent(id); }

        template <typename ComponentType> ComponentArray<ComponentType>& GetComponentArray() { return *m_ComponentManager.GetComponentArray<ComponentType>(); }
