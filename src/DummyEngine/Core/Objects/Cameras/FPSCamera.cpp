...
namespace DE
{
    ...
    FPSCamera::FPSCamera() { SetDir(Vec3(0.0f, 0.0f, -1.0f)); }
    FPSCamera::FPSCamera(Vec3 camera_position) : FPSCamera() { m_Position = camera_position; }
    ...
    void FPSCamera::MoveInWorld(const Vec3& delta) { m_Position += delta; }
    ...
    void FPSCamera::RotateY(float angle) { SetDir(glm::rotate(m_Direction, glm::radians(-angle), m_WorldUp)); }
    ...
    Mat4 FPSCamera::GetViewProjection() const { return GetProjectionMatrix() * GetViewMatrix(); }
    ...
    float FPSCamera::GetFarPlane() const { return m_FarPlane; }

    void FPSCamera::LookAt(Vec3 point) { m_Direction = glm::normalize(point - m_Position); }
    ...
    void FPSCamera::SetPos(Vec3 pos) { m_Position = pos; }
    ...
    void FPSCamera::SetAspect(float aspect) { m_Aspect = aspect; }
    void FPSCamera::SetNearPlane(float dist) { m_NearPlane = dist; }
    void FPSCamera::SetFarPlane(float dist) { m_FarPlane = dist; }
}  // namespace DE