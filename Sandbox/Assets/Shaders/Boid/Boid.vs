...
layout(location = 8) in mat4  i_InstanceTransform;
...
void main()
{
    ...
    gl_Position      = u_Camera.projection * u_Camera.view * i_InstanceTransform * vec4(i_Pos, 1.0);
    vs_out.FragPos   = vec3(i_InstanceTransform * vec4(i_Pos, 1.0));
    vs_out.Normal    = mat3(transpose(inverse(i_InstanceTransform))) * i_Normal;
