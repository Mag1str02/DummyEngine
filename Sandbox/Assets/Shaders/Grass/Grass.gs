...
void main()
{
    ...
    gl_Position = vp[u_VP].projection * vp[u_VP].view * (gl_in[0].gl_Position + vec4(v_Right[0] * GRASS_WIDTH / 3, 0.0) + vec4(v_Up[0] * GRASS_HEIGHT / 2, 0.0) + vec4(offset, 0.0) * 0.3);
    EmitVertex();
    ...
    gl_Position = vp[u_VP].projection * vp[u_VP].view * (gl_in[0].gl_Position + vec4(v_Up[0] * GRASS_HEIGHT, 0.0) + vec4(offset, 0.0));
    EmitVertex();
