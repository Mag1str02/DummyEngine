...
void main() {
    vec3 normalized_v_Normal = normalize(v_Normal);
    vec3 view_direction = normalize(u_CameraPos - v_FragPos);

    vec3 result = vec3(0.0);
    
    for (int i = 0; i < u_LightAmount.x; ++i) {
        result += DirectionalLightImpact(u_DirLights[i], normalized_v_Normal, view_direction);
    }
    for (int i = 0; i < u_LightAmount.y; ++i) {
        result += PointLightImpact(u_PointLights[i], normalized_v_Normal, view_direction, v_FragPos);
    }
    for (int i = 0; i < u_LightAmount.z; ++i) {
        result += SpotLightImpact(u_SpotLights[i], normalized_v_Normal, view_direction, v_FragPos);
    }
