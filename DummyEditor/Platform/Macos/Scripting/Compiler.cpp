...
namespace DummyEngine {
    ...
    class MacosCompilerImpl : public CompilerImpl {
        LOG_AUTHOR(Compiler)
    public:
        MacosCompilerImpl() { AddDefine("DE_PLATFORM_MACOS"); }
        virtual bool Compile(const Path& source, const Path& destination) {
            ...
            LOG_INFO("Compile command: {}", compile_command);
            int res = system(compile_command.c_str());
            LOG_INFO("Compile result: {}", res);
            return res == 0;
        }
        virtual bool Link(const std::vector<Path>& sources, const Path& destination, const std::string& library_name) {
            ...
            LOG_INFO("Link command: {}", link_command);
            int res = system(link_command.c_str());
            LOG_INFO("Link result: {}", res);
            return res == 0;
        }
    ...
    private:
        ...
        std::string AddPathDLL(const Path& destination, const std::string& library_name) {
            return " -o " + destination.string() + "/" + library_name + ".dylib";
        }
    ...
    Scope<CompilerImpl> Compiler::CreateCompilerImpl() {
        return CreateScope<MacosCompilerImpl>();
    }
